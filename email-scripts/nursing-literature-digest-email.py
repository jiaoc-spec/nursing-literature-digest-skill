#!/usr/bin/env python3
"""Send the daily nursing literature digest via Gmail SMTP."""
...
def build_email(digest_path: Path, date: str) -> MIMEMultipart:
    ...
    # Extract short summary from first ~20 lines for email preview
    lines = body.splitlines()
    preview_lines = [l for l in lines[:30] if l.strip()][:12]
    preview = "\n".join(preview_lines)

    msg = MIMEMultipart("alternative")
    msg["Subject"] = f"Pflegeliteratur-Digest {date}"
    msg["From"] = SENDER
    msg["To"] = RECIPIENT
